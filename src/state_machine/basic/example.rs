...
impl Automaton<i32> for ExampleAutomaton {
    ...
    // fn get_curr_state(&self) -> Box<dyn AutomatonState<i32>> {
    //     Self::get_start_state()
    // }
}

// mod example_automaton {

struct StartState;
impl AutomatonState<i32> for StartState {
    fn decide_next(&self, seed: &mut ThreadRng) -> Option<AutomatonEdge<i32>> {
        match (*seed).next_u32() % 100 {
